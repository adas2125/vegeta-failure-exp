...
// WorkersUsed returns the maximum number of workers used by the latest attack.
func (a *Attacker) WorkersUsed() uint64 {
	return atomic.LoadUint64(&a.workersUsed)
}
...
func (a *Attacker) Attack(tr Targeter, p Pacer, du time.Duration, name string) <-chan *Result {
	...
	atomic.StoreUint64(&a.workersUsed, workers)
