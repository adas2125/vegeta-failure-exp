...
func attackCmd() command {
	...
	fs.StringVar(&opts.workersOutput, "workers-output", "", "Write worker usage JSON to this file")
	fs.StringVar(&opts.workersTimelineOutput, "workers-timeline-output", "", "Write worker usage samples CSV to this file")
...
func attack(opts *attackOpts) (err error) {
	...
	var stopWorkerTimeline func() error
	if opts.workersTimelineOutput != "" {
		stopWorkerTimeline, err = startWorkerTimelineOutput(opts.workersTimelineOutput, atk, time.Second)
		if err != nil {
			return err
		}
	}

	processErr := processAttack(atk, res, enc, sig, pm)
	if stopWorkerTimeline != nil {
		if err := stopWorkerTimeline(); processErr == nil && err != nil {
			processErr = err
		}
	}
	if processErr != nil {
	...
	if opts.workersOutput != "" {
		return writeWorkersOutput(opts.workersOutput, opts.workers, opts.maxWorkers, atk.WorkersUsed())
	}
...
type workersOutput struct {
	InitialWorkers uint64 `json:"initial_workers"`
	MaxWorkers     uint64 `json:"max_workers"`
	WorkersUsed    uint64 `json:"workers_used"`
}

func writeWorkersOutput(path string, initial, max, used uint64) error {
	out, err := file(path, true)
	if err != nil {
		return err
	}
	defer out.Close()

	return json.NewEncoder(out).Encode(workersOutput{
...
func startWorkerTimelineOutput(path string, atk *vegeta.Attacker, interval time.Duration) (func() error, error) {
	out, err := file(path, true)
	if err != nil {
		return nil, err
	}
	...
	start := time.Now()
	writeSample := func(now time.Time) error {
		if _, err := fmt.Fprintf(writer, "%.9f,%d,%d\n", float64(now.UnixNano())/1e9, now.Sub(start).Milliseconds(), atk.WorkersUsed()); err != nil {
			return err
		}
		return writer.Flush()
	}

	if _, err := writer.WriteString("timestamp_unix,elapsed_ms,workers_used\n"); err != nil {
		out.Close()
		return nil, err
	}
	if err := writeSample(start); err != nil {
		out.Close()
		return nil, err
	}
	...
	go func() {
		...
		for {
			select {
			case now := <-ticker.C:
				if err := writeSample(now); err != nil {
					done <- err
					return
				}
			case <-stop:
				err := writeSample(time.Now())
